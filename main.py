...
Using the simpler, more effective implementation from src/digi.py
"""
import os
import sys
import argparse
...
class PetConfig:
    ...
    SPRITE_MAPPING = {
        ...
        "idle_left": [(1, 0), (1, 1), (1, 2), (1, 3)],
        "walk_right": [(0, 4), (0, 5), (0, 6), (0, 7)],
        "walk_left": [(1, 4), (1, 5), (1, 6), (1, 7)],
        "talk_right": [(0, 0), (0, 1), (0, 2), (0, 3)],
        "talk_left": [(1, 0), (1, 1), (1, 2), (1, 3)],
        "sleep_right": [(0, 4), (0, 5), (0, 6), (0, 7)],
        "sleep_left": [(1, 4), (1, 5), (1, 6), (1, 7)]
    }
...
class DesktopPet:
    def __init__(self, master):
        ...
        # Add a close button (small and discrete at bottom-right)
        close_btn = tk.Button(self.master, text="×", command=self.master.destroy, 
                            font=("Arial", 8), bg="lightgray", fg="black", 
                            width=2, height=1, relief=tk.FLAT)
        close_btn.place(x=self.width-18, y=self.height-18)
        ...
        self.master.after(1000, lambda: self.speak("Hello! I'm your desktop buddy!"))
    ...
    def load_sprites(self):
        ...
        try:
            ...
            if not os.path.exists(self.config.SPRITESHEET_PATH):
                # Try to find the spritesheet in other common locations
                alt_paths = [
                    "src/pet_sprites.png",
                    "resources/spritesheets/pet_sprites.png",
                    "resources/pet_sprites.png"
                ]
                
                found = False
                for path in alt_paths:
                    if os.path.exists(path):
                        print(f"Found spritesheet at alternate location: {path}")
                        self.config.SPRITESHEET_PATH = path
                        found = True
                        break
...
""")
        ...
        if old_config.WIDTH != self.config.WIDTH or old_config.HEIGHT != self.config.HEIGHT:
            ...
            for widget in self.master.winfo_children():
                if isinstance(widget, tk.Button) and widget["text"] == "×":
                    widget.place(x=self.width-18, y=self.height-18)
    ...
    def update_animation(self):
        ...
        self.frame = (self.frame + 1) % max(1, frames_count)
        self.update_sprite()
    ...
    def update_sprite(self):
        ...
        try:
            color = {
                "idle": "#87CEFA",  # Light blue
                "walk": "#90EE90",  # Light green
                "talk": "#FFB6C1",  # Light pink
                "sleep": "#D3D3D3"   # Light gray
            }.get(self.animation_state, "#87CEFA")
        except:
            color = "#87CEFA"
        outline_color = "black"
        
        # Draw a simple cartoon-like character as fallback
        body_x = self.width // 6
        ...
        # Draw eyes
        eye_size = max(5, min(10, body_width // 10))
        ...
        # Right eye
        right_eye_x = body_x + body_width * 2 // 3
        self.canvas.create_oval(
        ...
        # Draw pupils (position based on direction)
        ...
        # Left pupil
        self.canvas.create_oval(
            left_eye_x - pupil_size + pupil_offset_x, eye_y - pupil_size,
            left_eye_x + pupil_size + pupil_offset_x, eye_y + pupil_size,
            fill="black", tags="sprite"
        )
        
        # Right pupil
        self.canvas.create_oval(
            right_eye_x - pupil_size + pupil_offset_x, eye_y - pupil_size,
            right_eye_x + pupil_size + pupil_offset_x, eye_y + pupil_size,
            fill="black", tags="sprite"
        )
        ...
        mouth_width = body_width // 3
        
        if self.animation_state == "talk":
            # Open mouth for talking
            self.canvas.create_oval(
                body_x + body_width // 2 - mouth_width // 2,
            ...
            # Normal smile
            if self.frame % 2 == 0:
                # Slight variation based on animation frame
                mouth_curve = 3
            else:
                mouth_curve = 5
    ...
    def random_behavior(self):
        ...
        if choice < walk_prob:  # Chance to walk
            ...
            # Wake up after a random time
            wake_time = random.randint(3000, 8000)
            self.master.after(wake_time, lambda: setattr(self, 'animation_state', 'idle'))
    ...
    def speak(self, message):
        ...
        # Scale bubble size based on message length
        bubble_width = min(self.width - 20, max(100, len(message) * 10))
        bubble_height = 30 + (message.count('\n') * 15)
        ...
        self.speech_text = self.canvas.create_text(
            ...
            text=message, fill="black", font=("Arial", 9, "bold"), 
            width=bubble_width - 10, justify="center", tags="speech"
        )
        
        # Schedule removal of speech bubble
        self.speech_timer = self.master.after(5000, lambda: self.clear_speech(old_state))
    ...
    def reset_focus(self):
        ...
        self.master.after(100, lambda: self.master.attributes('-topmost', True))
...
def copy_sprite_if_needed():
    """Copy sprite file from src directory if it exists but is not in current directory."""
    if not os.path.exists("pet_sprites.png"):
        # Try to find sprites in various locations
        possible_paths = [
            "src/pet_sprites.png",
            "resources/spritesheets/pet_sprites.png",
            "resources/pet_sprites.png",
        ...
        for path in possible_paths:
            if os.path.exists(path):
                print(f"Copying sprites from {path} to current directory")
                shutil.copy2(path, "pet_sprites.png")
                break
...
def main():
    ...
    # Look for sprites and copy if needed
    copy_sprite_if_needed()
